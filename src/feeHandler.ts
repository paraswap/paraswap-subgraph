...
function calcToTokenFeeWithSlippage(
    ...
    if (fixedFeeBps.notEqual(BigInt.fromI32(0))) {
        feeShare = _calcFixedFees(
            slippage.notEqual(BigInt.fromI32(0))
                ? expectedAmount
                : receivedAmount,
            fixedFeeBps
        );
...
function _calcSlippage(
    ...
    return fixedFeeBps.le(BigInt.fromI32(50)) &&
        positiveAmount.gt(negativeAmount)
        ...
        if (version.notEqual(BigInt.fromI32(0))) {
            if (
                ...
                feeShare.partnerShare = feeShare.paraswapShare
                    .times(
                        feeBps.gt(BigInt.fromI32(10000))
...
export function calcFeeShareV2(
    ...
    // If fee = 0
    if (
        feeShare.paraswapShare
            .plus(feeShare.partnerShare)
            .equals(BigInt.fromI32(0))
    ) {
...
function calcCompleteFeeV2(
    ...
    if (takeSlippage) {
        ...
        feeShare.paraswapShare = currentParaswapShare.plus(
            halfPositiveSlippage
        );

        if (!positiveSlippageToUser) {
            let currentPartnerShare = feeShare.partnerShare;
            feeShare.partnerShare = currentPartnerShare.plus(
                halfPositiveSlippage
            );
        }
    }
