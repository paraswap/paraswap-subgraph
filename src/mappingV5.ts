...
export function handleSwappedV3(event: SwappedV3): void {
    ...
    if (partnerShare.gt(BigInt.fromI32(0))) {
        ...
        if (isReferralProgramBool) {
            let referrerFeeId =
                event.params.partner.toHex() + "-" + feeToken.toHex();
            ...
            if (!referrerFee) {
                ...
                referrerFee.totalRewards = referrerFee.totalRewards.plus(
                    partnerShare
                );
            }
            referrerFee.save();
        ...
        // PartnerFee entity
            ...
            if (!partnerFee) {
                ...
                partnerFee.totalRewards = partnerFee.totalRewards.plus(
                    partnerShare
                );
            }
            partnerFee.save();
...
export function handleSwapOnUniswapV2Fork(call: SwapOnUniswapV2ForkCall): void {
    ...
    if (
        ...
        if (lastPoolHexLength < 42) {
            log.error(
                "Invalid last pool hex {} for swapOnUniswapV2Fork tx {}",
...
export function handleSwapOnUniswapV2ForkWithPermit(
    ...
    if (
        ...
        if (poolsLength < 1) {
            log.error(
                "Invalid pools length on swapOnUniswapV2ForkWithPermit tx {}",
                [call.transaction.hash.toHex()]
            );
            return;
        }
...
export function handleBuyOnUniswapV2ForkWithPermit(
    ...
    if (
        ...
        if (poolsLength < 1) {
            log.error(
                "Invalid pools length on buyOnUniswapV2ForkWithPermit tx {}",
                [call.transaction.hash.toHex()]
            );
            return;
        }
